    ...
    try:
        async with session.get(sitemap_url) as response:
            if response.status == 200:
                ...
                tasks = [process_url(session, url, namespace, use_cache) for url in urls]
                ...
                if show_errors:
                    ...
                    nested_tasks = [fetch_sitemap(session, sitemap.find(f'{{{namespace}}}loc').text, False, use_cache) for sitemap in sitemaps]
...
async def process_url(session, url, namespace, use_cache):
    """
    ...
    title = name.text if name is not None and name.text else await fetch_title(session, loc, use_cache)
    return {'title': title if title else loc, 'link': loc}

async def fetch_title(session, url, use_cache):
    """
